// lib/presentation/screens/admin/admin_notifications_screen.dart
//
// SCREEN — ADMIN NOTIFICATIONS
// Real-time notification stream for the logged-in region admin.
// Providers: currentAdminProvider, notificationsProvider(uid)
...
class AdminNotificationsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
          ...
          if (adminUid.isNotEmpty)
            TextButton(
              onPressed: () async {
                ...
                try {
                  await repo.markAllNotificationsRead(adminUid);
                } catch (_) {}
              },
              child: const Text(
                'MARK ALL READ',
      ...
      body: adminUid.isEmpty
          ? const Center(child: CircularProgressIndicator())
          : _NotificationsList(adminUid: adminUid),
...
class _NotificationsList extends ConsumerWidget {
  const _NotificationsList({required this.adminUid});
  ...
  void _showDetail(BuildContext context, WidgetRef ref, NotificationModel notif) {
    showModalBottomSheet(
      ...
      builder: (_) => _AdminNotifDetailSheet(
        notification: notif,
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final notifsAsync = ref.watch(notificationsProvider(adminUid));
    ...
    return notifsAsync.when(
      ...
      error: (e, _) => Center(
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              onPressed: () => ref.invalidate(notificationsProvider(adminUid)),
      ...
      data: (notifs) {
        ...
        return RefreshIndicator(
          onRefresh: () async =>
              ref.invalidate(notificationsProvider(adminUid)),
...
// ── Admin notification detail sheet ──────────────────────────────────────────

class _AdminNotifDetailSheet extends StatelessWidget {
  const _AdminNotifDetailSheet({
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isRead ? AppColors.surface : AppColors.primaryBlue.withValues(alpha: 0.06),
          borderRadius: BorderRadius.circular(AppRadius.md),
          border: Border.all(
            color: isRead ? AppColors.border : AppColors.primaryBlue.withValues(alpha: 0.25),
          ),
        ),
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
